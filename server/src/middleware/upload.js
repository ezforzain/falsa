...
const ALLOWED_TYPES = new Set(['business-docs', 'products', 'store-banners', 'store-logos', 'avatars', 'labels', 'categories']);
...
function fileFilter(_req, file, cb) {
  // GIF is here specifically for animated promo/sale banners (see PromoBannerManager.jsx) — every
  // other image path (product photos, avatars, the single store banner) just quietly gains GIF
  // support too, which is harmless.
  const mimeOk = /^image\/(png|jpe?g|webp|gif|heic|heif)$|^application\/pdf$/.test(file.mimetype);
  ...
  cb(ok ? null : new Error('Only PNG, JPEG, WEBP, GIF, HEIC/HEIF, or PDF files are allowed.'), ok);
}
