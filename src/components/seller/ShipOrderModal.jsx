...
export default function ShipOrderModal({ open, order, bankComplete, pickupComplete, onClose, onShipped }) {
  ...
  const refreshTracking = async () => {
    ...
    try {
      ...
      setTrackingError(friendlyShippingError(err.message));
    } finally {
      ...
      <div className="relative w-full max-w-[480px] max-h-full overflow-y-auto bg-surface rounded-2xl shadow-2xl p-6 animate-fade-up">
        ...
        {!done && (
          <>
            ...
            {method === 'falsafah' && bankComplete && !pickupComplete && (
              <div className="border border-dashed border-border-strong rounded-lg p-4 text-center">
                <IconAlertCircle width="18" height="18" className="text-orange-text mx-auto mb-2" />
                <p className="text-[13px] text-ink-soft mb-3">
                  Add your pickup address, city, and a valid mobile number (11 digits, e.g. 03001234567) before shipping with
                  Falsafah — that's where and how TCS collects the parcel.
                </p>
                <Link
                  to="/seller/settings"
                  onClick={onClose}
            ...
            {method === 'falsafah' && bankComplete && pickupComplete && (
              <>
                ...
                <button
                  ...
                  disabled={loading}
                  className="w-full flex items-center justify-center gap-2 cursor-pointer disabled:cursor-not-allowed disabled:opacity-60 bg-green hover:bg-green-hover text-white font-semibold text-sm py-3 rounded-full shadow-[0_6px_16px_rgba(59,111,224,0.25)] transition-colors"
                >
                  {loading && (
        ...
        {done && shippedOrder.shippingMethod === 'falsafah' && (
          <div className="flex flex-col items-center gap-4">
            ...
            <div className="text-center">
              ...
              <p className="text-[13px] text-text-muted">
                Tracking number <strong className="text-ink">{shippedOrder.trackingId}</strong>
              </p>
            </div>
            ...
            <div className="w-full border-t border-border pt-3.5">
              <TrackingWidget tracking={tracking} loading={trackingLoading} error={trackingError} onRefresh={refreshTracking} size="full" />
            </div>
          </div>
        )}
      </div>
